...
class Reforcement_ai():
    ...
    def xiazi(self, playerjudger, color, step, table4=None):
        ...
        # 计算我方的分值
        a = np.array(self.covalue_table(self.table_2d, self.color_dict[self.color]))
        # 计算敌方的分值
        b = np.array(self.covalue_table(self.table_2d, self.color_dict[self.anticolor]))
        c = np.array([a, b])
        # 计算最大值所在的坐标
        pos = np.unravel_index(np.argmax(c), c.shape)
        print('算值')
        # 调整落子坐标
    ...
    def find8(self, tl4, color):
        VCF_points = []
        for i in range(4):
            for j in range(15):
                for k in range(15):
                    if tl4[i][j][k] == 8:
                        if self.playerjudger.check_forbidden([j + 1, i + 1], 'Black', False) == True or color == 1:
